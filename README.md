...
In practice this means a large falling or settled clump of one material (say, a 1000-grain block of Sand) costs work proportional to its outer shell each tick, not its full volume — the interior grains are "grouped" implicitly and ride along for free, and any grain that gets exposed (the group breaks apart, erodes, or lands) automatically rejoins individual simulation the moment that happens. Materials with a despawn timer (Fire, Smoke, Steam, Ash) are exempted from this and always stay active, since they need to keep counting down even while buried.

This is implemented via `_activate_if_exposed` in `Scripts/Singletons/SimulationGlobal.gd`, which replaces the plain `_activate` calls used whenever a cell gains or keeps an occupant.
